...
Keeps original JSON keys, only translates values.
"""
...
def translate_exercise(exercise):
    """Translate a complete exercise object to pt-BR, keeping original keys."""
    translated = {}
    ...
    for key in exercise.keys():
        ...
        # Translate lists of strings (muscles, instructions)
            ...
            if key == "instructions":
                ...
                translated[key] = [translate_text(inst) for inst in value]
            else:
                # Translate muscle names
                translated[key] = [translate_text(muscle) for muscle in value]
        elif value is None:
            translated[key] = None
